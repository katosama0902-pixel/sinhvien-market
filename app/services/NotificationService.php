...
class NotificationService
{
    ...
    public static function notifyStrikeReceived(int $userId, string $userEmail, string $userName, string $reason): void
    {
        self::notifModel()->create(
            $userId,
            'strike_warning',
            "⚠️ Bạn nhận 1 Gậy Cảnh Cáo",
            "Tài khoản của bạn vi phạm quy định. Lý do: $reason",
            '#' // Không có link cụ thể, chỉ để đọc
        );

        Mailer::send(
            $userEmail,
            $userName,
            '⚠️ Cảnh cáo Vi phạm Chính sách — SinhVienMarket',
            EmailTemplate::strikeWarning($userName, $reason)
        );
    }

    public static function notifyAccountSuspended(int $userId, string $userEmail, string $userName, string $reason): void
    {
        // Vẫn tạo notif in-app để khi họ hết hạn khóa, họ vào sẽ thấy
        self::notifModel()->create(
            $userId,
            'account_suspended',
            "⛔ Tài khoản bị khóa 7 ngày",
            "Lý do vi phạm gần nhất: $reason",
            '#'
        );
        ...
        Mailer::send(
            $userEmail,
            $userName,
            '⛔ Tài khoản của bạn đã bị khóa tạm thời — SinhVienMarket',
            EmailTemplate::accountSuspended($userName, $reason)
        );
    }

    public static function notifyAccountBanned(int $userId, string $userEmail, string $userName, string $reason): void
    {
        // Banned vĩnh viễn thì không cần tạo in-app notif vì họ không vào được nữa
        Mailer::send(
            $userEmail,
            $userName,
            '🚫 Thông báo Khóa Tài Khoản Vĩnh Viễn — SinhVienMarket',
            EmailTemplate::accountBanned($userName, $reason)
        );
    }
}
