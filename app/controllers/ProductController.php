...
class ProductController extends Controller
{
    ...
    public function show(): void
    {
        ...
        $appUrl = rtrim($_ENV['APP_URL'] ?? '', '/'); // BUG-A05 fix: bỏ hardcoded localhost fallback
        $og = [
            'title'       => $product['title'],
    ...
    public function create(): void
    {
        ...
        $price = null; // BUG-016 fix: khởi tạo trước để tránh undefined variable khi type != 'sale'
        if ($old['type'] === 'sale') {
        ...
        // ── AI Kiểm Duyệt (Feature #6) ───────────────────────────────
        if ($productId) {
            $aiReview = AIModerationService::reviewProduct($old['title'], $old['description']);
            
            try {
                // Lưu kết quả AI review qua method public của model (BUG-003 fix)
                $this->productModel->updateAiReview($productId, $aiReview['status'], $aiReview['reason']);
            } catch (\Throwable $e) {
                error_log('[AI Review] Failed to save review for product ' . $productId . ': ' . $e->getMessage());
            }

            if ($aiReview['status'] === 'safe') {
                $this->productModel->updateStatus($productId, 'active');
                NotificationService::notifyProductApproved($user['id'], $user['email'], $user['name'], $productId, $old['title']);
                Flash::set('success', 'Đăng bài thành công! AI đã tự động duyệt bài của bạn do nội dung an toàn.');
            } elseif ($aiReview['status'] === 'violation') {
                $this->productModel->updateStatus($productId, 'cancelled');
                NotificationService::notifyProductRejected($user['id'], $user['email'], $user['name'], $productId, $old['title'], "AI hệ thống phát hiện vi phạm: " . $aiReview['reason']);
                Flash::set('danger', 'Bài đăng của bạn bị từ chối tự động vì vi phạm quy định.');
            } else {
                // suspicious -> pending
                Flash::set('success', 'Đăng bài thành công! Bài đang chờ Admin duyệt tay do cần xác minh thêm.');
            }
        } else {
    ...
    public function suggestPrice(): void
    {
        ...
        if (mb_strlen($title) < 5) {
            $this->json(['success' => false, 'message' => 'Vui lòng nhập tên sản phẩm cụ thể hơn để AI phân tích.']);
            return;
        }

        $similarProducts = $this->productModel->getSimilarSoldPrices($title, $categoryId, 10);
        
        $prompt = "Tôi đang định bán một sản phẩm tên là '$title', tình trạng '$condition'.\n";
        if (!empty($similarProducts)) {
            $prompt .= "Lịch sử giá các sản phẩm tương tự đã bán trên chợ:\n";
            foreach ($similarProducts as $p) {
                $priceF = number_format($p['price']);
                $prompt .= "- {$p['title']} ({$p['condition']}): {$priceF}đ\n";
            }
        }
        $prompt .= "\nDựa trên dữ liệu trên và thị trường đồ cũ sinh viên, hãy gợi ý cho tôi một KHOẢNG GIÁ HỢP LÝ (vd: 150.000đ - 200.000đ) và giải thích ngắn gọn (dưới 50 chữ).";
