...
| Tuần 8 (19–23/04/2026) | Hardening & Admin | 2 | 0 | 0 | 2 |
| Tuần 9 (01–07/05/2026) | Payment Gateways & Realtime | 3 | 1 | 0 | 4 |
| **Tổng** | | **19** | **10** | **5** | **35** |

| Mức độ | Số lượng | Trạng thái |
|--------|----------|------------|
| 🔴 High (Crash/Blocker/Security) | 17 | ✅ Đã fix tất cả |
| 🟡 Medium | 10 | ✅ Đã fix tất cả |
| 🟢 Low (Visual/Warning) | 5 | ✅ Đã fix tất cả |
| **Tổng** | **32 bugs** | **100% resolved** |

---

## 🗓 Tuần 9 (01–07/05/2026) — Payment Gateways & Realtime

### [BF-031] Lỗi CSP chặn kết nối WebSocket Pusher và ảnh VietQR
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🔴 High (Blocker tính năng Chat và Thanh toán)
- **Mô tả:** Trình duyệt báo lỗi "Content Security Policy directive violates" khi tải thư viện Pusher JS và không hiện mã QR thanh toán.
- **Nguyên nhân:** Cấu hình `script-src` và `img-src` trong `index.php` thiếu domain của Pusher và VietQR.
- **Cách fix:** Bổ sung `https://js.pusher.com`, `wss://ws-ap1.pusher.com`, `https://img.vietqr.io` vào headers CSP.

### [BF-032] Lỗi truy cập method protected `execute()` từ Controller
- **Ngày phát hiện:** 06/05/2026
- **Mức độ:** 🔴 High (Crash ứng dụng)
- **Mô tả:** Gọi `$this->execute()` trực tiếp trong `TransactionController` gây lỗi `Call to protected method Core\Model::execute() from context 'App\Controllers\TransactionController'`.
- **Nguyên nhân:** Hàm `execute()` được đặt `protected` trong `Core\Model` theo thiết kế để chống SQL injection tùy tiện từ Controller, nhưng người code lại gọi thẳng nó.
- **Cách fix:** Đưa logic gọi SQL vào các hàm public trong class `App\Models\Transaction`.

### [BF-033] Spam HTTP Requests do cơ chế Polling cũ
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🟡 Medium (Hiệu năng Server)
- **Mô tả:** Hàm `setInterval` trong Chat View liên tục gửi request mỗi 3 giây khiến server bị quá tải (DDoS nhẹ).
- **Cách fix:** Thay thế HTTP Polling bằng Pusher JS WebSocket (Pub/Sub).

### [BF-034] Rác mã nguồn VNPay Sandbox không sử dụng
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🔴 High (Technical Debt)
- **Mô tả:** File `VnpayService.php` và các đoạn code liên đới làm nặng và rối controller trong khi dự án đã chốt dùng ZaloPay & MoMo.
- **Cách fix:** Xóa sạch toàn bộ rác VNPay khỏi View, Controller, Router, Service.

## Tuần 11: Enterprise Architecture & Hardening

### [BF-035] Oversell / Race Condition khi Đấu Giá
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🔴 Critical (Lỗi Logic DB)
- **Mô tả:** 2 người cùng bấm Mua tại 1 mili-giây dẫn đến việc 1 sản phẩm bị bán cho 2 người do PHP không khóa luồng đọc ghi.
- **Cách fix:** Bọc luồng `checkout()` trong `DB Transaction` và sử dụng `SELECT ... FOR UPDATE` để khóa row (Pessimistic Locking).

### [BF-036] Webhook cộng tiền 2 lần (Lack of Idempotency)
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🔴 Critical (Lỗi Logic Tài Chính)
- **Mô tả:** Ngân hàng hoặc MoMo gửi webhook (IPN) nhiều lần nếu mạng lag, hệ thống cứ nhận là update status thành `paid` nhiều lần, nguy cơ gây lặp xử lý logic sau này.
- **Cách fix:** Thêm check `Idempotency` (Kiểm tra `$tx['payment_status'] === 'paid'`) trước khi update, trả về success ngay nếu đã xử lý.

### [BF-037] "God Controller" gây khó bảo trì
- **Ngày phát hiện:** 07/05/2026
- **Mức độ:** 🟡 Medium (Technical Debt)
- **Mô tả:** `AdminController` phình to lên tới gần 1000 dòng (37KB), ôm đồm quá nhiều chức năng (SRP violation).
- **Cách fix:** Tách thành 8 Controller chuyên biệt (`AdminUserController`, `AdminProductController`, v.v.).
