...
class Rating extends Model
{
    ...
    /**
     * Lấy tất cả đánh giá của 1 người bán (Dành cho trang Public Profile)
     * Nếu bị ẩn (is_hidden = 1), che nội dung bình luận
     */
    public function getByRatee(int $userId): array
    {
        ...
        foreach ($ratings as &$r) {
            if ($r['status'] === 'hidden' && !empty($r['comment'])) {
                $r['comment'] = 'Bình luận này đã bị Admin ẩn do vi phạm tiêu chuẩn cộng đồng.';
            }
        }
        return $ratings;
    }
    ...
    public function getAllForAdmin(): array
    {
        return $this->query(
            'SELECT r.*, 
                    u_rater.name AS rater_name, u_rater.email AS rater_email,
                    u_ratee.name AS ratee_name, u_ratee.email AS ratee_email,
                    p.title AS product_title
             FROM ratings r
             JOIN users u_rater ON u_rater.id = r.rater_id
             JOIN users u_ratee ON u_ratee.id = r.ratee_id
             JOIN products p ON p.id = r.product_id
             ORDER BY r.created_at DESC'
        );
    }
     ...
     */
    public function toggleHide(int $id): bool
    {
        $this->execute("UPDATE ratings SET status = IF(status = 'visible', 'hidden', 'visible') WHERE id = ?", [$id]);
        return true;
    }
    ...
    {
        return $this->queryOne('SELECT * FROM ratings WHERE id = ? LIMIT 1', [$id]);
    }
