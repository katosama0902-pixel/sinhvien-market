...
HTML;
     ...
     * Cảnh cáo 1 Gậy
     */
    public static function strikeWarning(string $name, string $reason): string
    {
        $time = date('d/m/Y H:i:s');
        $content = self::h1('⚠️', 'Cảnh cáo Vi phạm Chính sách')
            . self::p("Xin chào <strong>{$name}</strong>,")
            . self::p("Tài khoản của bạn vừa nhận <strong>1 Gậy Cảnh Cáo (Strike)</strong> từ Ban Quản Trị SinhVienMarket do có hành vi vi phạm quy định của nền tảng.")
            . "<div style='background:#fef2f2;border-left:4px solid #ef4444;border-radius:8px;padding:16px;margin:16px 0;'>"
            . "<p style='margin:0 0 8px 0;color:#991b1b;font-size:14px;'><strong>Thời gian vi phạm:</strong> {$time}</p>"
            . "<p style='margin:0;color:#991b1b;font-size:14px;'><strong>Lý do vi phạm:</strong> " . nl2br(htmlspecialchars($reason, ENT_QUOTES)) . "</p>"
            . "</div>"
            . self::p("🚨 <strong>Lưu ý quan trọng:</strong>")
            . "<ul style='color:#374151;font-size:14px;line-height:1.6;padding-left:20px;'>"
            . "<li><strong>Nhận 2 gậy:</strong> Tài khoản sẽ bị khóa 7 ngày.</li>"
            . "<li><strong>Nhận 3 gậy:</strong> Tài khoản sẽ bị khóa vĩnh viễn.</li>"
            . "</ul>"
            . self::p("Vui lòng đọc kỹ Quy định cộng đồng và tuân thủ để tránh bị gián đoạn hoạt động trên nền tảng.");

        return self::wrap($content, "⚠️ Cảnh cáo Vi phạm Chính sách — SinhVienMarket");
    }

    /**
     * Khóa tài khoản 7 ngày (2 Gậy)
     */
    public static function accountSuspended(string $name, string $reason): string
    {
        $time = date('d/m/Y H:i:s');
        $content = self::h1('⛔', 'Tài khoản Bị Khóa 7 Ngày')
            . self::p("Xin chào <strong>{$name}</strong>,")
            . self::p("Tài khoản của bạn đã nhận <strong>2 Gậy Cảnh Cáo</strong> và sẽ bị khóa tạm thời trong vòng 7 ngày.")
            . "<div style='background:#fef2f2;border-left:4px solid #ef4444;border-radius:8px;padding:16px;margin:16px 0;'>"
            . "<p style='margin:0 0 8px 0;color:#991b1b;font-size:14px;'><strong>Thời gian xử phạt:</strong> {$time}</p>"
            . "<p style='margin:0;color:#991b1b;font-size:14px;'><strong>Lý do vi phạm gần nhất:</strong> " . nl2br(htmlspecialchars($reason, ENT_QUOTES)) . "</p>"
            . "</div>"
            . self::p("Trong thời gian này, bạn sẽ không thể đăng nhập hoặc thực hiện bất kỳ giao dịch nào trên hệ thống.")
            . self::p("🚨 Nếu tiếp tục vi phạm và nhận gậy thứ 3 sau khi được mở khóa, tài khoản của bạn sẽ bị <strong>Khóa vĩnh viễn</strong>.");

        return self::wrap($content, "⛔ Tài khoản của bạn đã bị khóa 7 ngày");
    }

    /**
     * Khóa tài khoản Vĩnh viễn (3 Gậy)
     */
    public static function accountBanned(string $name, string $reason): string
    {
        $time = date('d/m/Y H:i:s');
        $content = self::h1('🚫', 'Tài khoản Bị Khóa Vĩnh Viễn')
            . self::p("Xin chào <strong>{$name}</strong>,")
            . self::p("Chúng tôi rất tiếc phải thông báo rằng tài khoản của bạn đã bị <strong>Khóa Vĩnh Viễn</strong> do nhận đủ 3 Gậy cảnh cáo hoặc có hành vi vi phạm đặc biệt nghiêm trọng.")
            . "<div style='background:#7f1d1d;border-radius:8px;padding:16px;margin:16px 0;color:#fecaca;'>"
            . "<p style='margin:0 0 8px 0;font-size:14px;'><strong>Thời gian thi hành:</strong> {$time}</p>"
            . "<p style='margin:0;font-size:14px;'><strong>Lý do vi phạm:</strong> " . nl2br(htmlspecialchars($reason, ENT_QUOTES)) . "</p>"
            . "</div>"
            . self::p("Quyết định này là cuối cùng nhằm bảo vệ môi trường giao dịch an toàn và minh bạch cho cộng đồng sinh viên. Bạn không thể kháng cáo quyết định này.");

        return self::wrap($content, "🚫 Tài khoản của bạn đã bị khóa vĩnh viễn");
    }
}
