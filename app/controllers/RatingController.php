...
class RatingController extends Controller
{
    ...
    public function create(): void
    {
        ...
        // KIỂM DUYỆT AI (Feature 4 - AI Moderation)
        if ($comment !== '') {
            $prompt = "Bạn là hệ thống kiểm duyệt nội dung. Hãy kiểm tra bình luận sau đây có chứa từ ngữ thô tục, chửi thề, khiêu dâm, lừa đảo, hay xúc phạm không. Chỉ trả lời đúng một từ 'HOP_LE' nếu an toàn, hoặc 'VI_PHAM' nếu vi phạm tiêu chuẩn cộng đồng. Không trả lời thêm bất kỳ thứ gì khác. Bình luận: '{$comment}'";
            $aiResponse = \App\Services\GoogleAiService::askGemini($prompt);
            
            // Nếu AI xác định là vi phạm
            if (strpos(strtoupper($aiResponse), 'VI_PHAM') !== false) {
                Flash::set('danger', 'Trợ lý AI nhận thấy bình luận của bạn vi phạm tiêu chuẩn cộng đồng. Vui lòng sử dụng ngôn từ phù hợp hơn.');
                $this->redirect('transactions/history');
                return;
            }
        }
