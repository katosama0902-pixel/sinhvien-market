...
class Auction extends Model
{
    ...
    {
        return $this->queryOne('SELECT * FROM auctions WHERE id = ? FOR UPDATE', [$id]);
    }
    ...
    {
        // BUG-020 fix: current_price không phải cột DB — tính inline theo công thức:
        // current_price = start_price - FLOOR(elapsed_minutes / step_minutes) * decrease_amount
        // Auction kết thúc khi: giá tính được <= floor_price HOẶC đã chạy > 24h (1440 phút)
        return $this->query(
            'SELECT * FROM auctions WHERE status = "active" AND (
                (start_price - (FLOOR(TIME_TO_SEC(TIMEDIFF(NOW(), started_at)) / 60 / step_minutes) * decrease_amount)) <= floor_price
                OR
                TIME_TO_SEC(TIMEDIFF(NOW(), started_at)) / 60 > 1440
            )'
        );
    }
