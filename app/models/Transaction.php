...
class Transaction extends Model
{
    ...
    public function getByUser(int $userId): array
    {
        return $this->query(
            'SELECT t.*, 
                    COALESCE(p.title, "[Sản phẩm đã xóa]") AS product_title,
                    p.image AS product_image,
                    COALESCE(buyer.name,  "[Tài khoản đã xóa]") AS buyer_name,
                    COALESCE(seller.name, "[Tài khoản đã xóa]") AS seller_name
             FROM transactions t
    ...
    /**
     * Cập nhật ảnh xác nhận thanh toán (VietQR)
     */
    public function updatePaymentProof(int $id, string $proofName): void
    {
     ...
     */
    public function confirmBankPayment(int $id): void
    {
        $this->execute(
            'UPDATE transactions SET payment_status = "paid", payment_confirmed_at = NOW(), order_status = "shipping" WHERE id = ?',
            [$id]
        );
    }
    ...
    {
        // NEW-002 fix: đổi sang LEFT JOIN (consistent với getByUser BUG-017 fix)
        // Đảm bảo admin vẫn thấy giao dịch dù product/user bị xóa
        $sql = 'SELECT t.*,
                       COALESCE(p.title, "[Sản phẩm đã xóa]") AS product_title,
                       COALESCE(buyer.name,  "[Tài khoản đã xóa]") AS buyer_name,
                       COALESCE(seller.name, "[Tài khoản đã xóa]") AS seller_name
