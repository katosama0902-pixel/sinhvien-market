...
class Product extends Model
{
    ...
    {
        return $this->queryOne('SELECT * FROM products WHERE id = ? FOR UPDATE', [$id]);
    }
    ...
    /**
     * Lấy danh sách sản phẩm để xuất dữ liệu, có hỗ trợ lọc theo ngày
     */
    public function getAllForExport(?string $fromDate = null, ?string $toDate = null): array
    {
        $sql = 'SELECT p.id, p.title, p.description, p.type, p.status, p.price, p.condition, p.created_at, 
    ...
    /**
     * Lấy giá các sản phẩm đã bán tương tự (dùng cho AI Gợi Ý Giá - Feature #7)
     */
    public function getSimilarSoldPrices(string $keyword, int $categoryId = 0, int $limit = 10): array
    {
        $sql = 'SELECT p.price, p.title, p.condition
                FROM products p
                WHERE p.status = "sold"
                  AND p.type = "sale"
                  AND p.price > 0
    ...
    /**
     * Lưu kết quả kiểm duyệt AI vào DB
     * (BUG-003 fix: trước đây ProductController gọi protected execute() trực tiếp)
     */
    public function updateAiReview(int $id, string $status, string $note): void
    {
        $this->execute(
            'UPDATE products SET ai_review_status = ?, ai_review_note = ? WHERE id = ?',
            [$status, $note, $id]
        );
    }
}
